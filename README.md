...
прибора, крутит сигналы из YAML-сценария и отдаёт значения по TCP. Бэкенд ходит
...
Свой прибор — это YAML-сценарий (`dev/*.yaml`): имя, тип, переменные (`ctype`,
`mode` r/rw/w, опционально `signal`). Запуск отдельного прибора:
`uv run python -m devsim --scenario <файл> --port <порт>`.
