"""Движок симуляции — общий tick-цикл для тестовых приборов.

Аналог `io.tick()` из unidriver: периодически продвигает время и обновляет
значения регистров-сенсоров через их сигналы. Управляющие регистры (control)
и команды (command) сигналов не имеют — их значение держит оператор.
"""
