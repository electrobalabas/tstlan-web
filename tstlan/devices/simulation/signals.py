"""Сигналы — стратегии эволюции значения регистра во времени.

Паттерн Strategy: прибор не знает, *как* меняется значение регистра, — это
решает подставленный объект `Signal`. Паттерн Composite: `Sum` (и оператор
`+`) собирает сложный сигнал из простых, например «синус + шум».
"""
...
class Follow(Signal):
    ...
    Так ведомый регистр-«выход» отслеживает регистр-«уставку», который меняет
    оператор: `Follow(setpoint) + Noise(...)` — выход с измерительным шумом.
    """
