...
from tstlan.devices.device_profile import (
    DeviceProfile,
    device_from_profile,
    load_profile,
)
...
def build_simulated(profile: DeviceProfile, device_id: str = "sim") -> SimulatedDevice:
    signals = {name: build_signal(spec) for name, spec in profile.signals.items()}
    return SimulatedDevice(device_from_profile(profile, device_id), signals, HANDLE)
...
def serve(
    profile: DeviceProfile, host: str = "127.0.0.1", port: int = 0
) -> DeviceServer:
    io = InMemoryUnidriverIO()
    engine = SimulationEngine(io, [build_simulated(profile)], interval=_TICK_SECONDS)
...
def main(argv: list[str] | None = None) -> None:
    ...
    server = serve(load_profile(args.profile), args.host, args.port)
