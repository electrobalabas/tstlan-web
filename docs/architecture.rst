...
* :class:`tstlan.devices.net.client.LazySocketUnidriverIO` - клиент к
  :mod:`devsim`. Соединяется при первом обращении и один раз переподключается
  после обрыва.
* будущая интеграция с ``libunidriver.so`` должна реализовать тот же контракт, не
  меняя :class:`tstlan.devices.service.DeviceService` и роуты.
...
Заголовок ``Cache-Control: no-cache, no-transform`` нужен, чтобы dev-proxy или
nginx не буферизовали поток.
