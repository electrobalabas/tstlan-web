...
Событие несёт серверное время ``t``, чтобы точки потока и истории ложились на
одну ось, не завися от часов браузера.
...
История значений: фоновый сэмплер
(:func:`tstlan.devices.history.run_sampler`) раз в секунду снимает значения
всех приборов в кольцевые буферы в памяти (:mod:`tstlan.devices.history`).
``GET /devices/{id}/history`` отдаёт накопленное окно; фронтенд сидирует им
графики при открытии карточки и дальше дописывает живым потоком. В БД история
не пишется: это оперативное состояние приборов с ограниченным окном, а не
учётные данные. Недоступный прибор сэмплер пропускает, не останавливаясь.
